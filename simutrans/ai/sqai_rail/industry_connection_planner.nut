  ...
  if ( show_msg == 1 ) {
    ...
    local fd = f_dest.get_tile_list()
        ...
        if ( print_message_box == 1 || print_status == 1 ) {
          gui.add_message_at(our_player, "--> factory: " + f_src.get_name() + " (" + coord_to_string(fs[0]) + ")  to factory " + f_dest.get_name() + " (" + coord_to_string(fd[0]) + ")  --> good: " + good, world.get_time())
    ...
    if ( print_message_box == 1 || print_status == 1 ) {
      gui.add_message_at(player_x(1), "check_factory_links() --> link count factory " + f_src.get_name() + " (" + coord_to_string(fs[0]) + ") to factory " + f_dest.get_name() + " (" + coord_to_string(fd[0]) + "): " + link_count  + " # plan Player " + our_player.get_name(), f_src.get_tile_list()[0])
    }
