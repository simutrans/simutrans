...
void grund_t::mark_image_dirty() const
{
	...
	if (imageid != IMG_EMPTY) {
		...
		gfx->mark_img_dirty(imageid, scr_pos.x, scr_pos.y);
	}
}
...
#endif
{
	...
	if(image==IMG_EMPTY) {
		...
		if(dirty) {
			gfx->mark_rect_dirty_clip( xpos, ypos + raster_tile_width / 2, xpos + raster_tile_width - 1, ypos + raster_tile_width - 1 CLIP_NUM_PAR );
		}
	}
	else {
		if(get_typ()!=wasser) {
			...
			if (visible)  {
				gfx->draw_normal( get_image(), xpos, ypos, 0, true, dirty CLIP_NUM_PAR );
#ifdef SHOW_FORE_GRUND
				if (get_flag(grund_t::draw_as_obj)) {
					gfx->draw_blend( get_image(), xpos, ypos, 0, gfx->palette_lookup(COL_RED) | OUTLINE_FLAG |TRANSPARENT50_FLAG, true, dirty CLIP_NUM_PAR );
				}
#endif
				...
				if(  climate_corners != 0  &&  (!weg  ||  !weg->hat_gehweg())  ) {
					...
					if(  water_corners  ) {
						if(  slope  ) {
							gfx->draw_alpha( ground_desc_t::get_water_tile(slope, wasser_t::stage), ground_desc_t::get_beach_tile( slope, water_corners ), ALPHA_RED, xpos, ypos, 0, 0, true, dirty|wasser_t::change_stage CLIP_NUM_PAR );
							if(  ground_desc_t::shore  ) {
								...
								gfx->draw_normal( shore, xpos, ypos, 0, true, dirty CLIP_NUM_PAR );
							}
						}
						else {
							// animate
							gfx->draw_alpha( ground_desc_t::sea->get_image(0,wasser_t::stage), ground_desc_t::get_beach_tile( slope, water_corners ), ALPHA_RED, xpos, ypos, 0, 0, true, dirty|wasser_t::change_stage CLIP_NUM_PAR );
						}
					}
				}
			...
			if(  underground_mode != ugm_all  ) {
				gfx->draw_normal( ground_desc_t::sea->get_image(get_image(),wasser_t::stage), xpos, ypos, 0, true, dirty|wasser_t::change_stage CLIP_NUM_PAR );
			}
			else {
				gfx->draw_blend( ground_desc_t::sea->get_image(get_image(),wasser_t::stage), xpos, ypos, 0, TRANSPARENT50_FLAG, true, dirty|wasser_t::change_stage CLIP_NUM_PAR );
			}
			return;
		}
	}
...
#endif
			gfx->draw_img_aux( ground_desc_t::get_marker_image( hang, false ), xpos, ypos, 0, true, dirty CLIP_NUM_PAR );
			if(  !ist_karten_boden()  ) {
				...
				if(  pos.z > gr->get_hoehe()  ) {
					...
					for(  sint8 z = pos.z - 1;  z > gr->get_hoehe();  z--  ) {
						gfx->draw_img_aux( ground_desc_t::get_marker_image(0, false), xpos, ypos - tile_raster_scale_y( (z - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
					}
					//display front part of marker for ground
					gfx->draw_img_aux( ground_desc_t::get_marker_image( gr->get_grund_hang(), false ), xpos, ypos - tile_raster_scale_y( (gr->get_hoehe() - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
				}
...
uint8 grund_t::display_obj_bg(const sint16 xpos, const sint16 ypos, const bool is_global, const bool draw_ways, const bool visible CLIP_NUM_DEF ) const
{
	...
	if(  visible  ) {
		...
		if(  is_global  &&  get_flag( grund_t::marked )  ) {
			gfx->draw_normal( ground_desc_t::get_marker_image( get_grund_hang(), true ), xpos, ypos, 0, true, dirty CLIP_NUM_PAR );
			if(  !ist_karten_boden()  ) {
				const grund_t *gr = welt->lookup_kartenboden(pos.get_2d());
				...
				if(  pos.z < gr->get_disp_height()  ) {
					//display back part of marker for grunds in between
					for(  sint8 z = pos.z + 1;  z < gr->get_disp_height();  z++  ) {
						gfx->draw_normal( ground_desc_t::get_marker_image(0, true), xpos, ypos - tile_raster_scale_y( (z - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
					}
					//display back part of marker for ground
					gfx->draw_normal( ground_desc_t::get_marker_image( gr->get_grund_hang() | gr->get_weg_hang(), true ), xpos, ypos - tile_raster_scale_y( (gr->get_hoehe() - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
				}
			}
		}
...
void grund_t::display_obj_fg(const sint16 xpos, const sint16 ypos, const bool is_global, const uint8 start_offset CLIP_NUM_DEF ) const
{
	...
	if(  is_global  &&  get_flag( grund_t::marked )  ) {
		gfx->draw_normal( ground_desc_t::get_marker_image( get_grund_hang(), false ), xpos, ypos, 0, true, dirty CLIP_NUM_PAR );
		if(  !ist_karten_boden()  ) {
			const grund_t *gr = welt->lookup_kartenboden(pos.get_2d());
			...
			if(  pos.z > gr->get_hoehe()  ) {
				//display front part of marker for grunds in between
				for(  sint8 z = pos.z - 1;  z > gr->get_hoehe();  z--  ) {
					gfx->draw_normal( ground_desc_t::get_marker_image( 0, false ), xpos, ypos - tile_raster_scale_y( (z - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
				}
				//display front part of marker for ground
				gfx->draw_normal( ground_desc_t::get_marker_image( gr->get_grund_hang(), false ), xpos, ypos - tile_raster_scale_y( (gr->get_hoehe() - pos.z) * TILE_HEIGHT_STEP, raster_tile_width ), 0, true, true CLIP_NUM_PAR );
			}
		}
	}
}
...
void grund_t::display_overlay(const sint16 xpos, const sint16 ypos)
{
	...
	if( schiene_t::show_reservations &&  hat_wege()  ) {
		if( weg_t* w0 = get_weg_nr(0) ) {
			weg_t* w1 = get_weg_nr(1);
			signal_t* sig = NULL;
			weg_t *signal_w = NULL;

			if (w0->has_signal()) {
				sig = this->find<signal_t>();
				signal_w = w0;
			}
			else if (w1 && w1->has_signal()) {
				sig = this->find<signal_t>();
				signal_w = w1;
			}
			...
			if(sig) {
				...
				ribi_t::ribi signal_mask = signal_w->get_ribi_maske();
				if( !signal_mask ) {
					signal_mask = signal_w->get_ribi_unmasked();
				}

				if (sig->get_two_ways()) {
					// Display only second arrow always green
					ribi_t::ribi signal_active_ribi = sig->get_dir();
					gfx->draw_signal_direction(xpos, ypos + tile_raster_scale_y(signal_w->get_yoff(), gfx->get_current_tile_raster_width()),
						signal_w->get_ribi_unmasked(), signal_mask&~signal_active_ribi, c1, c2, signal_w->is_diagonal(), get_weg_hang());
					signal_mask = signal_active_ribi;
				}
				...
				ribi_t::ribi way_ribi = signal_w->get_ribi_unmasked();
				gfx->draw_signal_direction(xpos, ypos + tile_raster_scale_y(signal_w->get_yoff(), gfx->get_current_tile_raster_width()),
					way_ribi, signal_mask, c1, c2, signal_w->is_diagonal(), get_weg_hang());
			}
			else if( w0->get_ribi_maske() ) {
				PIXVAL c1 = gfx->palette_lookup( COL_BLUE+2 );
				PIXVAL c2 = gfx->palette_lookup( COL_BLUE );
				gfx->draw_signal_direction( xpos, ypos + tile_raster_scale_y( w0->get_yoff(), gfx->get_current_tile_raster_width() ),
					w0->get_ribi_unmasked(), w0->get_ribi_maske(), c1, c2, w0->is_diagonal(), get_weg_hang() );
			}
			else if (w1  &&  w1->get_ribi_maske()) {
				PIXVAL c1 = gfx->palette_lookup(COL_BLUE + 2);
				PIXVAL c2 = gfx->palette_lookup(COL_BLUE);
				gfx->draw_signal_direction(xpos, ypos + tile_raster_scale_y(w1->get_yoff(), gfx->get_current_tile_raster_width()),
					w1->get_ribi_unmasked(), w1->get_ribi_maske(), c1, c2, w1->is_diagonal(), get_weg_hang());
			}
		}
	}
	...
	if(alter_weg==NULL) {
		...
		if((flags&has_way1)==0) {
			...
			if (weg->needs_crossing(other->get_desc())) {
				...
				if (!cr_desc  &&  w2 == water_wt) {
					// special permit for water
					cr_desc = crossing_logic_t::get_crossing(weg->get_waytype(), w2, 25, other->get_max_speed(), welt->get_timeline_year_month());
				}
				if (!cr_desc) {
