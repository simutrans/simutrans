...
void convoi_info_t::init(convoihandle_t cnv)
{
	...
	for (int cost = 0; cost<convoi_t::MAX_CONVOI_COST; cost++) {
		uint16 curve = chart.add_curve( gfx->palette_lookup(cost_type_color[cost]), cnv->get_finance_history(), convoi_t::MAX_CONVOI_COST, cost, MAX_MONTHS, cost_type_money[cost], false, true, cost_type_money[cost]*2 );
	...
	speed_bar.add_color_value(&mean_convoi_speed, gfx->palette_lookup(COL_GREEN));
