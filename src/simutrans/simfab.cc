...
{
	if(  !consumer.is_contained(ziel)  ) {
		consumer.insert_ordered( ziel, RelativeDistanceOrdering(pos.get_2d()) );
...
void fabrik_t::remove_consumer(koord ziel)
{
	consumer.remove(ziel);
}
...
void fabrik_t::rdwr(loadsave_t *file)
{
	...
	if(  file->is_saving()  ) {
		...
		anz_lieferziele = consumer.get_count();
...
DBG_DEBUG("fabrik_t::rdwr()","loading factory '%s'",s);
	...
	if (file->is_version_atleast(122, 1)) {
		...
		file->rdwr_long(delta_amount);
		file->rdwr_long(delta_amount_remainder);
	...
	for(int i=0; i<anz_lieferziele; i++) {
		if(file->is_loading()) {
			consumer.append(koord::invalid);
		}
		consumer[i].rdwr(file);
	}

	if(  file->is_version_atleast(112, 2)  ) {
		file->rdwr_long( consumer_active_last_month );
	}
...
sint32 fabrik_t::get_input_stock(const goods_desc_t *typ)
{
	sint32 menge = -1;
...
sint32 fabrik_t::get_output_stock(const goods_desc_t *typ)
{
	sint32 menge = -1;
...
bool fabrik_t::is_active_consumer( koord k ) const
{
	if ( consumer.is_contained(k) ) {
		return 0 < ( ( 1 << consumer.index_of(k) ) & consumer_active_last_month );
	}
	return false;
}
...
//			}
	...
	/// Compute base production.
	{
		...
		const uint64 want_prod_long = (uint64)prodbase * (uint64)boost * (uint64)delta_t + (uint64)delta_amount_remainder;
		prod = (uint32)(want_prod_long >> remainder_bits);
		delta_amount_remainder = (uint32)(want_prod_long & ((1 << remainder_bits) - 1 ));
	}
...
void fabrik_t::verteile_waren(const uint32 product)
{
	...
	for(  unsigned i=0;  i<plan->get_haltlist_count();  i++  ) {
		...
		for(  uint32 n=0;  n<consumer.get_count();  n++  ) {
			// this way, the halt, that is tried first, will change. As a result, if all destinations are empty, it will be spread evenly
			const koord lieferziel = consumer[(n + output[product].index_offset) % consumer.get_count()];
...
void fabrik_t::finish_rd()
{
	...
	if(  welt->get_settings().is_crossconnect_factories()  ) {
		...
		for(uint32 i=0; i<consumer.get_count(); i++) {
			fabrik_t * fab2 = fabrik_t::get_fab(consumer[i]);
			if(fab2) {
				fab2->add_supplier(pos.get_2d());
				consumer[i] = fab2->get_pos().get_2d();
			}
			else {
				// remove this ...
				dbg->warning( "fabrik_t::finish_rd()", "No factory at expected position %s!", consumer[i].get_str() );
				consumer.remove_at(i);
				i--;
			}
		}
	}
...
void fabrik_t::display_status(sint16 xpos, sint16 ypos)
{
	...
	if( input.get_count() ) {
		if(  currently_producing  ) {
			gfx->draw_box3d_clipped(xpos-2,  ypos-1, D_WAITINGBAR_WIDTH*input.get_count()+4, 6, gfx->palette_lookup(10), gfx->palette_lookup(12));
		}
		gfx->draw_rect_clipped(xpos-1, ypos, D_WAITINGBAR_WIDTH*input.get_count()+2, 4, gfx->palette_lookup(150), true CLIP_NUM_DEFAULT);
		...
		for(uint32 i=0; i < input.get_count(); i++) {
			...
			const sint64 pfactor = (sint64)desc->get_supplier(i)->get_consumption();
			...
			if (storage_capacity) {
				...
				if (currently_producing) {
					gfx->draw_rect_clipped(xpos, ypos - v - 1, 1, v, gfx->palette_lookup(COL_GREY4), true CLIP_NUM_DEFAULT);
					gfx->draw_rect_clipped(xpos + 1, ypos - v - 1, D_WAITINGBAR_WIDTH - 2, v, goods_color, true CLIP_NUM_DEFAULT);
					gfx->draw_rect_clipped(xpos + D_WAITINGBAR_WIDTH - 1, ypos - v - 1, 1, v, gfx->palette_lookup(COL_GREY1), true CLIP_NUM_DEFAULT);
				}
				else {
					gfx->tint_rect(xpos + 1, ypos - v - 1, D_WAITINGBAR_WIDTH - 2, v, goods_color, 60);
					gfx->mark_rect_dirty_wc(xpos + 1, ypos - v - 1, xpos + D_WAITINGBAR_WIDTH - 1, ypos - 1);
				}
			}
	...
	if( output.get_count() ) {
		if(  currently_producing  ) {
			gfx->draw_box3d_clipped(xpos-2,  ypos-1, D_WAITINGBAR_WIDTH*output.get_count()+4, 6, gfx->palette_lookup(10), gfx->palette_lookup(12));
		}
		gfx->draw_rect_clipped(xpos-1, ypos, D_WAITINGBAR_WIDTH*output.get_count()+2, 4, gfx->palette_lookup(COL_ORANGE), true CLIP_NUM_DEFAULT);
