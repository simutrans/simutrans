...
function test_convoy_state_schedule_editor()
{
	local pl = player_x(0)

	ASSERT_EQUAL(command_x(tool_build_way).work(pl, coord3d(2, 2, 0), coord3d(2, 8, 0), "cobblestone_road"), null)
	ASSERT_EQUAL(command_x(tool_build_station).work(pl, coord3d(2, 3, 0), "BusStop"), null)
	...
	{
		// nobody has a schedule dialog open: never reported as being edited,
		// neither without a schedule nor after a script assigned one - a
		// script-side change_schedule is applied at once, it is not a session
		ASSERT_TRUE(cnv.is_in_depot())
		ASSERT_FALSE(cnv.is_schedule_editor_open())
		cnv.change_schedule(pl, convoy_state_schedule(wt_road, [ coord3d(2, 7, 0), coord3d(2, 3, 0) ]))
		ASSERT_FALSE(cnv.is_schedule_editor_open())
	}

	// the GUI case (dialog open => true) cannot be reached from a script;
	// what must hold is that leaving the depot never reports a stale session
